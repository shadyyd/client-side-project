...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!isLoggedIn()) {
    ...
    function addQuestion(index, question = null) {
      ...
      const correctAnswersList = document.getElementById(
      ...
      if (question) {
        ...
        question.options.forEach((option, j) => {
          document.querySelector(
            `#optionsList${index} .form-group:nth-child(${j + 1}) input`
          ).value = option.text;
        });
        question.options.forEach((answer, j) => {
          if (answer.isCorrect) {
            document.querySelector(
              `#correctAnswersList${index} .form-check-input[value="${String(
                j
              )}"]`
            ).checked = true;
          }
      ...
      // Example usage:
      document
        .querySelector(`#questionContainer${index} .delete-question`)
    ...
    function deleteQuestion(index) {
      ...
      questionContainers.forEach((container, i) => {
        ...
        const questionTextInput = container.querySelector(
          ".form-control[id^='questionText']"
        );
        questionTextInput.id = `questionText${i + 1}`;

        const questionTypeSelect = container.querySelector(
        ...
        optionsContainer.id = `optionsContainer${i + 1}`;

        const optionsList = container.querySelector("div[id^='optionsList']");
        optionsList.id = `optionsList${i + 1}`;

        const correctAnswersList = container.querySelector(
          "div[id^='correctAnswersList']"
        );
        correctAnswersList.id = `correctAnswersList${i + 1}`;
      });
    }
